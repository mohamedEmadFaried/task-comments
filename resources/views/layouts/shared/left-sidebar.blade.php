...
<div class="left-side-menu">
    ...
    <div class="h-100" data-simplebar>
        ...
        <div id="sidebar-menu">
            ...
            <ul id="side-menu">
                    ...
                    <div class="collapse" id="sidebarProductManagement">
                        ...
                        <ul class="nav-second-level">
                            ...
                            @if (App\Helpers\Helper::adminCan('admin.product.index'))
                                @php $count = App\Models\Product::where('status','!=',1)->count(); @endphp
                                <li>
                                    <a href="{{ route('admin.product.index') }}">{{ __('Products') }}<span
                                            class="badge badge-success badge-pill float-right">{{ $count }}</span></a>
                                </li>
                            @endif
                            @if (App\Helpers\Helper::adminCan('admin.product-comments'))
                                @php $count = App\Models\ProductReview::where('status','!=',1)->count(); @endphp
                                <li>
                                    <a href="{{ route('admin.product-comments') }}">{{ __('Products Comments') }}<span
                                            class="badge badge-success badge-pill float-right">{{ $count }}</span></a>
                                </li>
                            @endif
                    ...
                    <div class="collapse" id="sidebarVendorManagement">
                        <ul class="nav-second-level">
                            ...
                            @if (App\Helpers\Helper::adminCan('admin.store-reject-status.index'))
                                @php $count = App\Models\Store::where('approved','!=',1)->count(); @endphp

                                <li>
                                    <a href="{{ route('admin.store-reject-status.index') }}">{{ __('Stores are awaiting approval') }}<span
                                            class="badge badge-success badge-pill float-right">{{ $count }}</span></a>
                                </li>
                            @endif
                        </ul>
                ...
                <li>
                    ...
                    <div class="collapse" id="sidebarSales">
                        <ul class="nav-second-level">
                            ...
                            @endif
                            @if (App\Helpers\Helper::adminCan('admin.report.total'))
                                <li>
                                    <a href="{{ route('admin.report.total') }}">{{ __('Report Sales') }}</a>
                                </li>
                            @endif
                </li>
            </ul>
                ...
                <li>
                    <a href="#sidebarWholeSales" data-toggle="collapse">
                        <i data-feather="airplay"></i>
                        {{-- <span class="badge badge-success badge-pill float-right">4</span> --}}
                        <span> {{ __('Wholesale Orders') }} </span>
                        <span class="menu-arrow"></span>
                    </a>
                    <div class="collapse" id="sidebarWholeSales">
                        <ul class="nav-second-level">
                            <li>
                                @if (App\Helpers\Helper::adminCan('admin.wholesale-order.index'))
                            <li>
                                <a href="{{ route('admin.wholesale-order.index') }}">{{ __('Wholesale Orders') }}</a>
                            </li>
                            @endif

                            @if (App\Helpers\Helper::adminCan('admin.report.whole-sales.total'))
                                <li>
                                    <a href="{{ route('admin.report.whole-sales.total') }}">{{ __('Report Wholesale') }}</a>
                                </li>
                            @endif
        ...
        @endif
        @if (App\Helpers\Helper::adminCan('admin.page.index'))
            <li>
        ...
        @endif
        @if (App\Helpers\Helper::adminCan('admin.setting.index'))
            <li>
    ...
    @endif
    @if (App\Helpers\Helper::adminCan('admin.notification.index'))
        <li>
    ...
    @endif
    @if (App\Helpers\Helper::adminCan('admin.advertisement.index'))
        <li>
